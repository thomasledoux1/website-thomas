...
const Home = () => {
  ...
  const renderCase = ({ index, url, altLogo, logo, content, tags }) => {
    return (
      <div data-aos={`fade-${index % 2 === 0 ? 'right' : 'left'}`} className="p-8 shadow-case rounded-lg hover:shadow-case-hover transition transition-shadow transition-duration-300 ease-in-out">
        <div className="portfolio-case h-full">
          <a className="flex flex-col h-full" href={url}>
            ...
            {content}
            <div className="portfolio-tagsWrapper flex flex-col mt-4 flex-grow justify-end">
              <ul className="portfolio-tags flex">
                {tags.map((tag, i) =>
                  <li className="bg-gray-400 py-1 px-4 mr-2 rounded-md" key={i}>{tag}</li>
                )}
              </ul>
            </div>
          </a>
  ...
  return (
    <Layout>
      ...
      <section id="hero" className="container mx-auto grid md:grid-cols-2 min-h-screen-without-nav items-center content-center">
        ...
        <div data-aos="fade-left" className="hero-image">
          <img className="personal-picture__img rounded-full lazy" alt="profile picture" src="https://res.cloudinary.com/dzrea5zhv/image/upload/w_320/e_blur:1000,q_1,f_auto/me_qvrwky.jpg" data-src="https://res.cloudinary.com/dzrea5zhv/image/upload/v1583171588/me_qvrwky.jpg" />
        </div>
        <span onClick={scrollTo} className="absolute bottom-0 right-1/2 transform -translate-x-1/2 cursor-pointer">
          <FontAwesomeIcon icon={faAngleDown} />
        </span>
      </section>
      <section id="personal" className="container mx-auto grid md:grid-cols-2 min-h-screen items-center content-center" ref={personalRef}>
      ...
      <section id="portfolio" className="container mx-auto min-h-screen items-center content-center">
        <h2 className="text-center mb-8">Some of my work</h2>
      ...
      <section id="contact" className="container grid md:grid-cols-3 mt-8 gap-8  min-h-screen-without-nav">
